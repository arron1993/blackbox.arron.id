        ...
        while True:
            try:
                ...
                if (self.api.get_session_status() != SessionStatus.OFF.value and
                    ...
                    is_in_pitlane = self.api.get_is_in_pitlane()
                    if (was_in_pitlane == 1 and is_in_pitlane == 0) and self.api.get_is_engine_running():
                        # was in the pitlane but has now left
                        # checking if the engine is running because sometimes
                        # the game teleports you out of the pitlane
                        Event("onNewStint")
